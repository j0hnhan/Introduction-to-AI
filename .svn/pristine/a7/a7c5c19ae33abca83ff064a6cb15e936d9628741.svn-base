  ...
  def getAction(self, gameState):
    ...
    def exp(self,gameState,depth,agentIndex,agentNum):
        ...
        for action in legalActions:
            succssor = gameState.generateSuccessor(agentIndex,action)
            if agentIndex == agentNum:
                v = expmaximize(self,succssor,depth-1,0,agentNum)
            else:
                v = exp(self,succssor,depth,agentIndex+1,agentNum)
            expValue += p*v
        return expValue
...
def betterEvaluationFunction(currentGameState):
  ...
  eval -= 3* curFoodNum
  minGhostDis = 100
  for s in curGhostStates:
      minGhostDis = min(minGhostDis,manhattanDistance(curPos,s.getPosition()))
  eval += 2 * max(minGhostDis, 3)
  scaredTime = 0
  for time in curScaredTimes:
      scaredTime += time
  if scaredTime != 0:
      eval += 2*scaredTime
  eval -= 2.75*len(curCapsule)
  return eval
